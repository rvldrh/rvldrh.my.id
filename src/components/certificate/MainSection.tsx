...
import { certifcateItems } from "@/data/certificate";

const CertficateSection = ({ id }: { id: string }) => {
  return (
      ...
      >
        <ConstrainedBox classNames="p-4 py-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
            {certifcateItems.map((item, index) => (
              <div
              key={index}
              ...
              >
                {item.type === "video" ? (
                  ...
                  <img
                    src={item.src}
                    alt={item.alt}
                    className="w-full h-80 object-cover transition duration-300 group-hover:brightness-50"
                  />
                )}
